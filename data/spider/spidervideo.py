...
for page in range(1, 6):
    ...
    print(page)
    spiderhtml(url, movieList)
    time.sleep(10)

for movie in movieList:
    query = f"INSERT INTO video (VideoID, Title, Description, PictureURL, VideoURL, Date) VALUES ('{movie['id']}', '{movie['title']}', '{movie['content']}', '{movie['picurl']}', '{movie['videourl']}','{movie['updatetime']}')"
    print(query)
    # 打印出后在sql文件内插入字段
